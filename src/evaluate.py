...
def list_people(cfg: EvalConfig) -> List[Path]:
    ...
    return sorted(
        [
            p
            for p in cfg.enroll_dir.iterdir()
            if p.is_dir()
        ]
    )
...
def sweep_thresholds(
    ...
    for thr in thresholds:
        far = (
            float(np.mean(impostor <= thr))
            if impostor.size
            else 0.0
        )

        frr = (
            float(np.mean(genuine > thr))
            if genuine.size
            else 0.0
        )
...
def main():
    ...
    for pdir in people_dirs:
        ...
        embs = load_embeddings_for_person(
            embedder,
            pdir,
            cfg,
        )
        ...
        if len(embs) >= cfg.min_imgs_per_person:
            ...
            print(
                f"Skipping {name}: only {len(embs)} "
                f"valid aligned crops "
    ...
    if len(names) < 1:
        print(
            "Not enough data to evaluate. Enroll more samples."
        )
        return
    ...
    print(
        f"Genuine (same person): "
        f"{describe(genuine)}"
    )

    print(
        f"Impostor (diff persons): "
        f"{describe(impostor)}"
    )
    ...
    if best is not None:
        ...
        print(
            f"\nSuggested threshold "
            f"(target FAR {cfg.target_far * 100:.1f}%): "
            f"thr={thr:.2f} "
            f"FAR={far * 100:.2f}% "
            f"FRR={frr * 100:.2f}%"
        )
        ...
        print(
            f"\nNo threshold in range met "
            f"FAR <= {cfg.target_far * 100:.1f}%. "
            "Try widening threshold sweep range or "
            "collecting more varied samples."
        )
