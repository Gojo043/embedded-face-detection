...
def mean_embedding(
    ...
    E = np.stack(
        [e.reshape(-1) for e in embeddings],
        axis=0,
    ).astype(np.float32)
...
def main():
    ...
    if base_samples:
        print(
            f"Re-enroll mode: found {len(base_samples)} "
            f"existing samples in {person_dir}/"
        )

    print(
        "Tip: stable lighting, move slightly left/right, different expressions."
    )
    ...
    try:
        while True:
            ...
            if key == ord("r"):
                ...
                status_msg = (
                    "NEW samples reset (existing kept)."
                )
            ...
            if key == ord("s"):
                ...
                if total < max(
                    ...
                    status_msg = (
                        f"Not enough total samples to save (have {total})."
                    )
                ...
                meta = {
                    ...
                    "note": (
                        "Embeddings are L2-normalized vectors. "
